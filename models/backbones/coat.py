...
class ConvRelPosEnc(nn.Module):
    ...
    def forward(self, q, v, size):
        ...
        B, h, _, Ch = v_img.shape
        v_img = v_img.reshape(B, h, H, W, Ch).permute(0, 1, 4, 2, 3).reshape(B, -1, H, W)
        conv_v_img = torch.cat([c(x) for c, x in zip(self.conv_list, v_img.split(self.channel_splits, dim=1))], dim=1)
        conv_v_img = conv_v_img.reshape(B, h, -1, H, W).permute(0, 1, 3, 4, 2).reshape(B, h, H * W, -1)  # Shape: [B, h*Ch, H, W] -> [B, h, H*W, Ch].
