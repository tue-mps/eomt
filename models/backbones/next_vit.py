...
class MHCA(nn.Module):
    ...
    def __init__(self, out_channels, head_dim):
        ...
        norm_layer = partial(nn.BatchNorm2d, eps=NORM_EPS)
        self.groups = out_channels // head_dim
...
class NTB(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + out.transpose(1, 2).reshape(B, C, H, -1)
...
@register_model
def nextvit_small_cus(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=drop_path_rate, drop=dropout, **kwargs)
    return model


@register_model
def nextvit_base_cus(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=drop_path_rate, drop=dropout, **kwargs)
    return model


@register_model
def nextvit_large_cus(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
